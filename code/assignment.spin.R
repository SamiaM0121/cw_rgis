...
# Reference: Chapter 2
library(readr)
df_fish <- read_csv("https://example.com/data/data_finsync_nc.csv")
...
v_s1 <- df_site_w_county %>%
  group_by(county) %>%
  summarize(n_sites = n(), .groups = "drop") %>%
  filter(n_sites > 0) %>%
  pull(county)
...
ggplot() +
  ...
  geom_sf(data = sf_site, color = "red", size = 2) +
  coord_sf() +
  theme_minimal() +
  labs(
    title = "Fish Survey Sites by County",
    subtitle = "Counties with at least one site (shaded) and survey sites (red dots)",
    x = "Longitude",
    y = "Latitude"
  )
...
df_county_areas <- sf_county_s1 %>%
  mutate(county_area = st_area(geometry)) %>%
  as_tibble() %>%
  select(county, county_area)

df_county_areas %>%
  arrange(desc(county_area)) %>%
  slice(1)